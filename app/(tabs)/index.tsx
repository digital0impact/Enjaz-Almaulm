                  ...
                  <ThemedView style={[styles.dataSection, { backgroundColor: 'transparent' }]}>
                    ...
                    <ThemedView style={styles.actionsGrid}>
                      <TouchableOpacity 
                        ...
                        onPress={async () => {
                          try {
                            // تحميل بيانات الأداء المهني من التخزين
                            ...
                            // حساب المتوسط العام
                            const weightedSum = performanceData.reduce((acc, item) => acc + (item.score * (item.weight || 10)), 0);
                            const totalWeight = performanceData.reduce((acc, item) => acc + (item.weight || 10), 0);
                            const overallAverage = Math.round(weightedSum / totalWeight);
                            ...
                            router.push({
                              ...
                              params: {
                                ...
                                overallGrade: getScoreLevel(overallAverage)
                              }
                            });
                          } catch (error) {
                            console.log('Error loading performance data:', error);
                            Alert.alert('خطأ', 'لا يمكن تحميل بيانات التقرير');
                          }
                        }}
                      >
