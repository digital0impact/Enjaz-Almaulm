...
### التحقق من ربط متجر سلة بـ Supabase

للتأكد من أن الويب هوك وجداول Supabase مربوطان بشكل صحيح، راجع **`docs/salla-supabase-verify.md`**. يتضمن قائمة تحقق، سكربت تحقق من الجداول (`scripts/verify-salla-supabase.sql`)، وطريقة اختبار استدعاء الدالة يدوياً (مثلاً بـ curl).
