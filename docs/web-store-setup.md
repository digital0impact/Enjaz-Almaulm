...
- **التأخير:** التطبيق يعيد المحاولة تلقائياً عدة مرات بعد العودة من صفحة الدفع. إن لم يظهر الاشتراك، استخدم زر **«تحقق من اشتراكي»** أو حدّث الصفحة بعد ثوانٍ أو تحقق من لوحة Supabase أن السجل قد أُضيف في جدول `subscriptions`.
...
**لتفعيل Realtime لجدول `subscriptions` في Supabase:**

1. من لوحة Supabase: **Database** → **Replication** (أو **Publications**).
2. تأكد من وجود منشور `supabase_realtime`.
3. أضف جدول **`subscriptions`** إلى قائمة الجداول المُستنسخة (انقر **Add table** واختر `public.subscriptions`).

بعد ذلك، عند إنشاء سجل اشتراك جديد عبر الـ Webhook، سيستقبل التطبيق التحديث فوراً ويعرض الاشتراك الجديد. يمكن أيضاً استخدام زر **«تحقق من اشتراكي»** في صفحة الاشتراكات للتحقق يدوياً في أي وقت.
